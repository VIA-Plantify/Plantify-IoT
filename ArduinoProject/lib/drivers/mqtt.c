                                  ...
                                  const char *password)
{
    ...
    if ((uint8_t)(2 + remaining) > buf_size) return 0;
...
uint8_t mqtt_raw_connect(void)
{
    ...
    if (!pkt_len) { printf("SUBSCRIBE build failed.\n"); return 0; }

    printf("Subscribing: %s\n", _sub_topic);
    wifi_command_TCP_transmit(pkt, pkt_len);

    for (uint16_t i = 0; i < 200; i++) { _delay_ms(10); if (_rx_ready) break; }
...
uint8_t mqtt_raw_publish(const char *payload)
{
    ...
    if (!pkt_len) { printf("PUBLISH build failed.\n"); return 0; }
...
void mqtt_handle_incoming(void)
{
    ...
    if (data[0] == 0x30)
    {
        ...
        if (topic_len < remaining)
        {
            ...
            if      (strcmp(payload_str, "led_on")  == 0) led_on(1);
            else if (strcmp(payload_str, "led_off") == 0) led_off(1);
            else if (strcmp(payload_str, "beep")    == 0) buzzer_beep();
            else if (strncmp(payload_str, "pump_on_", 8) == 0)
                pump_run_for((uint8_t)atoi(&payload_str[8]) * 1000);
            else printf("Unknown CMD: %s\n", payload_str);
        }
    }
    else if (data[0] == 0x20) printf("CONNACK\n");
    else if (data[0] == 0x90) printf("SUBACK\n");
    else if (data[0] == 0xD0) printf("PINGRESP\n");
